...
#define BOOXIN_SDL_WINDOW_VULKAN 0x0000000010000000ull
#define BOOXIN_SDL_WINDOW_RESIZABLE 0x00000020ull
#define BOOXIN_SDL_WINDOW_HIGH_PIXEL_DENSITY 0x00002000ull

/** BooxinGlues / Mesa Zink: desktop GL 3.3+ for MC 26.3+ RenderPearl. */
static int pearl_zink_mode(void) {
    const char *v;
    v = getenv("BOOXIN_GLUES_PEARL");
    if (v && v[0] == '1') return 1;
    v = getenv("BOOXIN_SDL_DESKTOP_GL");
    if (v && v[0] == '1') return 1;
    v = getenv("GALLIUM_DRIVER");
    if (v && strcmp(v, "zink") == 0) return 1;
    v = getenv("MESA_LOADER_DRIVER_OVERRIDE");
    if (v && strcmp(v, "zink") == 0) return 1;
    return 0;
}
...
static int booxin_sdl_gl_set_attribute(int attr, int value) {
    int orig = value;
    if (pearl_zink_mode()) {
        /* Desktop GL for RenderPearl: keep CORE / 3.3+ as requested. */
        if (attr == BOOXIN_SDL_GL_CONTEXT_PROFILE_MASK) {
            if (value != BOOXIN_SDL_GL_CONTEXT_PROFILE_CORE &&
                value != BOOXIN_SDL_GL_CONTEXT_PROFILE_ES) {
                value = BOOXIN_SDL_GL_CONTEXT_PROFILE_CORE;
            }
        } else if (attr == BOOXIN_SDL_GL_CONTEXT_MAJOR_VERSION) {
            if (value < 3) value = 3;
        } else if (attr == BOOXIN_SDL_GL_ACCUM_RED_SIZE ||
                   (attr > BOOXIN_SDL_GL_ACCUM_RED_SIZE &&
                    attr <= BOOXIN_SDL_GL_STEREO)) {
            return 1;
...
static uint64_t sanitize_window_flags(uint64_t flags) {
    ...
    /*
     * SDL3: OPENGL (0x2) + VULKAN (0x10000000) → "Conflicting window graphics flags".
     * BooxinGlues: prefer Vulkan 1.2 for RenderPearl — OSMesa+system EGL cannot
     * create a coherent desktop-GL context (glGetError mismatch / Adreno GLES).
     * Other GLES translators: prefer OPENGL.
     */
    if (pearl_zink_mode()) {
        if (out & BOOXIN_SDL_WINDOW_OPENGL) {
            LOGI("CreateWindow pearl: OPENGL→VULKAN flags was 0x%llx",
                 (unsigned long long)out);
            out &= ~BOOXIN_SDL_WINDOW_OPENGL;
        }
        out |= BOOXIN_SDL_WINDOW_VULKAN;
    } else if ((out & BOOXIN_SDL_WINDOW_OPENGL) && (out & BOOXIN_SDL_WINDOW_VULKAN)) {
        LOGW("CreateWindow strip VULKAN (keep OPENGL) flags was 0x%llx",
             (unsigned long long)out);
        out &= ~BOOXIN_SDL_WINDOW_VULKAN;
    } else if (!(out & BOOXIN_SDL_WINDOW_VULKAN)) {
        out |= BOOXIN_SDL_WINDOW_OPENGL;
    }
    return out;
}
...
int booxin_sdl_force_gles(void *sdl_handle) {
    ...
    const int pearl = pearl_zink_mode();
    ...
    if (!egl || !egl[0]) egl = pearl ? "libEGL.so" : gl;

    int red = 0;
    if (pearl) {
        /* OSMesa/Zink is not MobileGlues — do not redirect system EGL into it. */
        LOGI("pearl/zink mode: skip MG EGL redirect; egl=%s gl=%s",
             egl ? egl : "?", gl ? gl : "?");
    } else {
        red = redirect_libegl_to_mobileglues();
    ...
    if (set_hint_pri) {
        if (pearl) {
            /* Vulkan RenderPearl path — do not force EGL/GLES for the window. */
            set_hint_pri("SDL_OPENGL_ES_DRIVER", "0", BOOXIN_SDL_HINT_OVERRIDE);
            set_hint_pri("SDL_VIDEO_FORCE_EGL", "0", BOOXIN_SDL_HINT_OVERRIDE);
        } else {
        ...
        if (pearl) {
            set_hint("SDL_OPENGL_ES_DRIVER", "0");
            set_hint("SDL_VIDEO_FORCE_EGL", "0");
        } else {
    ...
    if (pearl) {
        LOGI("default Vulkan window path (pearl/BooxinGlues — skip GL CORE defaults)");
        return 0;
    }
