...
class LaunchCommandBuilder(
    ...
    fun build(
        ...
        if (renderer == GlRendererKind.BOOXIN_GLUES || renderer == GlRendererKind.BOOXIN_ZINK) {
            require(MinecraftJavaRequirement.usesSdlWindowing(mcVersionId)) {
                "BooxinGlues 仅支持 Minecraft 26.3+（当前 $mcVersionId）"
            }
        }
        ...
        val pathA: com.booxin.launcher.core.runtime.BooxinGlResolved? = null
        ...
        // 远古版：Java 8 Cacio 走 -Xbootclasspath/p；现代版：Java 17 Cacio 走 /a + agent。
        val cacioBootJars = if (legacyLwjgl) {
            ...
            android.util.Log.i(
                "LaunchCmd",
                "legacy LWJGL2/AWT → Cacio8 bootclasspath/p jars=${cacio.joinToString { it.name }}"
            )
            cacio
        } else {
            emptyList()
        }
        val cacio17BootJars = if (!legacyLwjgl) {
            val cacio = AndroidGameRuntime.cacio17BootJars()
            require(cacio.isNotEmpty()) {
                "现代版需要 Cacio17 AWT: ${AndroidGameRuntime.cacio17Dir().absolutePath}"
            }
            val agent = AndroidGameRuntime.cacio17AgentJar()
            require(agent.isFile && agent.length() > 0L) {
                "缺少 cacio-agent: ${agent.absolutePath}"
            }
            android.util.Log.i(
                "LaunchCmd",
                "modern AWT → Cacio17 bootclasspath/a jars=${cacio.joinToString { it.name }}"
            )
            cacio
        } else {
            emptyList()
        ...
        // OSHI：禁止把单独的 oshi-android-sysfs.jar 放进 -cp。
        // 该 jar 若仍是旧版（class 69）会抢在 oshi-core 之前加载并直接崩；
        // Fabric Knot 也常忽略前置补丁。只改写 libraries 里的 oshi-core。
        ...
        for (lib in resolvedLibraries) {
            ...
            if (file != null) {
                val isOshiCore = lib.name.startsWith("com.github.oshi:oshi-core:")
                val toAdd = if (isOshiCore) {
                    AndroidGameRuntime.materializePatchedOshiCore(context, file) ?: file
                } else {
                    file
                }
                classpath += toAdd
        ...
        var existingClasspath = classpath.filter { it.isFile && it.length() > 0L }
            // 遗留独立补丁 jar 一律剔除（可能是 class 69）。
            .filterNot { it.name.equals("oshi-android-sysfs.jar", ignoreCase = true) }
        ...
        if (isForgeBootstrap) {
            ...
            val curatedWithOshi = curated.map { file ->
                if (file.name.startsWith("oshi-core", ignoreCase = true)) {
                    AndroidGameRuntime.materializePatchedOshiCore(context, file) ?: file
                } else {
                    file
                }
            ...
            val prefix = existingClasspath.filter { file ->
                ...
                n.contains("lwjgl") || n.contains("sdl") || n.contains("jni") ||
                    n.contains("booxin") || n.contains("oshi-core-android") ||
                    file.absolutePath.contains("android-lwjgl", true)
            }
            existingClasspath = (prefix + curatedWithOshi).distinctBy { file ->
        ...
        val jvmArgsWithAwt = if (legacyLwjgl) {
            // Java 8 Cacio：系统属性 + bootclasspath/p。
            ...
            // Java 17+：用 Caciocavallo CTCToolkit，避免桌面 libawt_xawt。
            // 模块参数须 option=value（JNI CreateJavaVM）。
            val boot = cacio17BootJars.joinToString(File.pathSeparator) { it.absolutePath }
            val agent = AndroidGameRuntime.cacio17AgentJar().absolutePath
            jvmArgs + listOf(
                "-Djava.awt.headless=false",
                "-Dcacio.managed.screensize=${windowWidth}x${windowHeight}",
                "-Dcacio.font.fontmanager=sun.awt.X11FontManager",
        ...
        if (MinecraftJavaRequirement.usesSdlWindowing(mcVersionId)) {
            ...
            val zinkDesktop =
                glKind == GlRendererKind.BOOXIN_GLUES ||
                    glKind == GlRendererKind.BOOXIN_ZINK ||
                    glKind == GlRendererKind.VULKAN_ZINK
            if (zinkDesktop) {
                // 26.3：窗口走 Vulkan；OSMesa 仍挂 LIBGL，避免 bootstrap 去找 libGLX。
                envBase["SDL_OPENGL_ES_DRIVER"] = "0"
                envBase["SDL_VIDEO_FORCE_EGL"] = "0"
                envBase["BOOXIN_SDL_DESKTOP_GL"] = "1"
                envBase["BOOXIN_GLUES_PEARL"] = "1"
                envBase["BOOXIN_PEARL_VULKAN"] = "1"
                envBase["BOOXIN_VULKAN_LIB"] = "libvulkan.so"
                envBase["LIBGL_EGL"] = "libEGL.so"
            } else {
                // SDL + 系统 EGL → ES；上下文再同步进当前 GLES 翻译库。
                envBase["SDL_OPENGL_ES_DRIVER"] = "1"
                envBase["SDL_VIDEO_FORCE_EGL"] = "1"
                envBase["SDL_OPENGL_LIBRARY"] = glLib.absolutePath
                envBase["SDL_EGL_LIBRARY"] =
                    if (glKind == GlRendererKind.REL) glLib.absolutePath else libGlEgl
            }
    ...
    /** Exact upgraded path only — do not silently substitute a newer artifact version. */
    private fun resolveLibraryFile(lib: ResolvedLibrary): File? {
        ...
        val parts = lib.name.substringBefore('@').split(':')
        if (parts.size >= 3) {
    ...
    private fun buildCommonAndroidJvmArgs(
        ...
        return buildList {
            ...
            // 必须给真实 .so；空着会找 libGLX.so.0，bootstrap 直接当致命错误。
            val glLibName = RuntimeEnv.glLibraryFile(
                AndroidGameRuntime.nativesDir(),
                renderer
            ).absolutePath
            add("-Dorg.lwjgl.opengl.libname=$glLibName")
            // Distant Horizons / sqlite-jdbc: force Android Bionic native (not Linux glibc).
            add("-Dorg.sqlite.lib.path=$nativeDir")
            add("-Dorg.sqlite.lib.name=libsqlitejdbc.so")
            // Shade/relocation in some mods still reads the same keys under dh_sqlite.
            add("-Ddh_sqlite.lib.path=$nativeDir")
            add("-Ddh_sqlite.lib.name=libsqlitejdbc.so")
            ...
            // SDL 26.3+：不要提前 load GLFW bridge，否则 ClassLoader 不对。
            if (!sdlWindowing) {
            ...
            // GLES 路径别设 vulkan.libname（会空 loader NPE）。BooxinGlues / Zink 需要。
            if (renderer == GlRendererKind.VULKAN_ZINK ||
                renderer == GlRendererKind.BOOXIN_GLUES ||
                renderer == GlRendererKind.BOOXIN_ZINK
            ) {
