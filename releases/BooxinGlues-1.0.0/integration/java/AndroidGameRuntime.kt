...
object AndroidGameRuntime {
    ...
    private const val ASSET_CACIO17_PREFIX = "app_runtime/caciocavallo17/"
    private const val ASSET_CACIO17_VERSION = "app_runtime/caciocavallo17/version"
    private const val ASSET_OSHI_SYSFS = "app_runtime/oshi/oshi-android-sysfs.jar"
    /** Stamp for rewritten oshi-core; bump to invalidate bad cached jars. */
    private const val ASSET_OSHI_SYSFS_VERSION = "oshi-core-inject-v5-assets"
    private const val OSHI_PATCH_CLASS = "oshi/hardware/platform/linux/LinuxCentralProcessor.class"
    /** Java 17 class major; Minecraft 1.20.x cannot load newer. */
    private const val OSHI_MAX_CLASS_MAJOR = 61
    ...
    /** Java 8 Cacio — 远古版会构造 java.awt.Frame。 */
    fun cacioDir(): File = File(LauncherPaths.runtimeDir, "caciocavallo")

    /** Java 17+ Cacio（CTCToolkit），供现代版模组初始化 AWT/Swing。 */
    fun cacio17Dir(): File = File(LauncherPaths.runtimeDir, "caciocavallo17")

    fun cacioClasspathJars(): List<File> = cacioBootJars()

    /**
     * Java 8 Cacio 走 `-Xbootclasspath/p:`：
    ...
    /**
     * Java 17+ Cacio 走 `-Xbootclasspath/a:`（JDK 9 起无 /p）。
     * shared → tta；agent 单独用 `-javaagent`。
     */
    fun cacio17BootJars(): List<File> {
        val dir = cacio17Dir()
        return listOf(
            File(dir, "cacio-shared-1.19.1-SNAPSHOT.jar"),
            File(dir, "cacio-tta-1.19.1-SNAPSHOT.jar")
        ).filter { it.isFile && it.length() > 0L }
    }

    fun cacio17AgentJar(): File = File(cacio17Dir(), "cacio-agent.jar")

    /** 覆盖 oshi LinuxCentralProcessor：避免 /sys memlat AccessDenied 拖垮游戏。 */
    fun oshiSysfsPatchJar(): File = File(LauncherPaths.runtimeDir, "oshi/oshi-android-sysfs.jar")

    /**
     * 从 APK assets 读取 Java17 补丁 class，写入 [original] 的副本。
     * 绝不把独立的 oshi-android-sysfs.jar 放进 classpath（旧文件可能是 class 69）。
     */
    fun materializePatchedOshiCore(context: Context, original: File): File? {
        if (!original.isFile || original.length() <= 0L) return null
        val patchClass = readOshiPatchClassFromAssets(context) ?: return null
        val major = classFileMajor(patchClass) ?: return null
        if (major > OSHI_MAX_CLASS_MAJOR) {
            android.util.Log.e(
                "BooxinRuntime",
                "asset oshi patch class major=$major exceeds $OSHI_MAX_CLASS_MAJOR; refuse"
            )
            return null
        }
        val destDir = File(LauncherPaths.runtimeDir, "oshi").also { it.mkdirs() }
        val dest = File(
            destDir,
            "oshi-core-android-${original.nameWithoutExtension}-${ASSET_OSHI_SYSFS_VERSION}.jar"
        )
        val marker = File(destDir, "${dest.name}.stamp")
        val stamp = "${original.length()}:${original.lastModified()}:$ASSET_OSHI_SYSFS_VERSION:$major"
        if (dest.isFile && dest.length() > 0L &&
            marker.takeIf { it.isFile }?.readText()?.trim() == stamp
        ) {
            val existing = readJarClassMajor(dest, OSHI_PATCH_CLASS)
            if (existing != null && existing <= OSHI_MAX_CLASS_MAJOR) {
                return dest
            }
            android.util.Log.w("BooxinRuntime", "cached oshi jar class major=$existing; rebuilding")
            dest.delete()
            marker.delete()
        }
        return runCatching {
            rewriteJarReplacingClass(original, dest, OSHI_PATCH_CLASS, patchClass)
            val written = readJarClassMajor(dest, OSHI_PATCH_CLASS)
            if (written == null || written > OSHI_MAX_CLASS_MAJOR) {
                dest.delete()
                error("written oshi class major=$written")
            }
            marker.writeText(stamp)
            android.util.Log.i(
                "BooxinRuntime",
                "oshi-core patched → ${dest.name} (class major=$written, base=${original.name})"
            )
            dest
        }.onFailure {
            android.util.Log.e("BooxinRuntime", "oshi-core patch failed: ${it.message}")
            dest.delete()
            marker.delete()
        }.getOrNull()
    }
    ...
    fun ensure(context: Context) {
        ...
        ensureLwjgl(context)
        ensureCacio(context)
        ensureCacio17(context)
        ensureOshiSysfsPatch(context)
        ensureJna(context)
        ensureNatives(context)
        stageJnaDispatch()
        // 不要在 Application / 主界面路径拷贝 sqlite / glibc 别名（曾把主线程卡死成黑屏）。
        // 见 [prepareGameNativeExtras]，仅在启动游戏前调用。
    }

    /**
     * Game-process / launch-only: Android sqlitejdbc + lightweight glibc soname aliases.
     * Must NOT run on the UI Application cold start path.
     */
    fun prepareGameNativeExtras(context: Context) {
        stageAndroidSqliteJdbc(context)
        stageGlibcSonameAliases(context)
    }

    /**
     * Distant Horizons / xerial sqlite-jdbc: HotSpot reports os.name=Linux so the
     * loader extracts glibc Linux/aarch64 natives (need libm.so.6). Stage the
     * official Android Bionic build and point org.sqlite.lib.path at it.
     */
    private fun stageAndroidSqliteJdbc(context: Context) {
        val dest = File(nativesDir().also { it.mkdirs() }, "libsqlitejdbc.so")
        val marker = File(nativesDir(), ".sqlitejdbc-android")
        val expected = "sqlite-jdbc-3.47.2.0-android-v1"
        val abi = preferredAbiFolder()
        val assetPath = "app_runtime/sqlite/$abi/libsqlitejdbc.so"
        val needCopy = !dest.isFile || !marker.isFile || marker.readText().trim() != expected
        if (needCopy) {
            val ok = runCatching {
                ...
                android.util.Log.w("BooxinRuntime", "sqlitejdbc android stage failed: ${it.message}")
                false
            }
            if (!ok) return
            android.util.Log.i(
                "BooxinRuntime",
                "staged Android libsqlitejdbc.so abi=$abi size=${dest.length()}"
            )
        }
        runCatching {
            val cacheCopy = File(context.cacheDir, "libsqlitejdbc.so")
            if (!cacheCopy.isFile || cacheCopy.length() != dest.length()) {
                dest.copyTo(cacheCopy, overwrite = true)
                cacheCopy.setReadable(true, false)
                cacheCopy.setExecutable(true, false)
            }
        }
    }

    /**
     * Only small Bionic libs as soname aliases (never copy libc — multi-MB and
     * froze the launcher UI when done on Application.onCreate).
     */
    private fun stageGlibcSonameAliases(context: Context? = null) {
        val destDirs = buildList {
            add(nativesDir().also { it.mkdirs() })
            context?.cacheDir?.let { add(it.also { d -> d.mkdirs() }) }
        }
        // 清掉误拷的巨型 libc 别名，避免占满磁盘 / 再次拖慢 IO。
        for (dest in destDirs) {
            listOf("libc.so.6", "libpthread.so.0", "librt.so.1").forEach { name ->
        ...
        val aliases = listOf(
            "libm.so.6" to "libm.so",
            "libdl.so.2" to "libdl.so"
        )
        var staged = 0
        for (dest in destDirs) {
            for ((aliasName, realName) in aliases) {
                val out = File(dest, aliasName)
                if (out.isFile && out.length() > 0L && out.length() < 2_000_000L) {
                    staged++
                    continue
                }
                val src = candidates.asSequence()
                    .map { File(it, realName) }
                    .firstOrNull { it.isFile && it.length() > 0L && it.length() < 2_000_000L }
                    ?: continue
                runCatching {
                    ...
                    android.util.Log.w(
                        "BooxinRuntime",
                        "glibc alias $aliasName failed: ${it.message}"
                    )
                }
            }
        }
        if (staged > 0) {
            android.util.Log.i("BooxinRuntime", "glibc soname aliases ready count=$staged")
        }
    }

    /** 删掉遗留的独立补丁 jar，避免被误加入 -cp。补丁 class 只从 assets 注入 oshi-core。 */
    private fun ensureOshiSysfsPatch(context: Context) {
        val destDir = File(LauncherPaths.runtimeDir, "oshi").also { it.mkdirs() }
        File(destDir, "oshi-android-sysfs.jar").delete()
        File(destDir, "oshi-android-sysfs.jar.tmp").delete()
        File(destDir, "sysfs-patch-version").delete()
        // 丢弃可能含 class 69 的旧注入产物（stamp 不含 v5）。
        destDir.listFiles()?.forEach { f ->
            val n = f.name
            if (n.startsWith("oshi-core-android-") && n.endsWith(".jar") &&
                !n.contains(ASSET_OSHI_SYSFS_VERSION)
            ) {
                f.delete()
                File(destDir, "$n.stamp").delete()
            }
        }
        // Touch assets once so missing asset fails early in logs.
        runCatching {
            val bytes = readOshiPatchClassFromAssets(context)
            val major = bytes?.let { classFileMajor(it) }
            android.util.Log.i("BooxinRuntime", "oshi asset patch ready major=$major")
        }.onFailure {
            android.util.Log.e("BooxinRuntime", "oshi asset patch missing: ${it.message}")
        }
    }

    private fun readOshiPatchClassFromAssets(context: Context): ByteArray? {
        return runCatching {
            context.assets.open(ASSET_OSHI_SYSFS).use { input ->
                java.util.zip.ZipInputStream(input.buffered()).use { zis ->
                    while (true) {
                        val entry = zis.nextEntry ?: break
                        if (entry.name == OSHI_PATCH_CLASS) {
    ...
    private fun readJarClassMajor(jar: File, classPath: String): Int? {
        if (!jar.isFile || jar.length() <= 0L) return null
        return runCatching {
            ZipFile(jar).use { zip ->
                val entry = zip.getEntry(classPath) ?: return@runCatching null
                zip.getInputStream(entry).use { input ->
                    val header = ByteArray(8)
                    if (input.read(header) < 8) return@runCatching null
                    classFileMajor(header)
                }
            }
        }.getOrNull()
    }
    ...
    private fun classFileMajor(classBytes: ByteArray): Int? {
        if (classBytes.size < 8) return null
        // CAFEBABE, minor(u16), major(u16)
        if (classBytes[0] != 0xCA.toByte() || classBytes[1] != 0xFE.toByte() ||
            classBytes[2] != 0xBA.toByte() || classBytes[3] != 0xBE.toByte()
        ) {
            return null
        }
        return ((classBytes[6].toInt() and 0xff) shl 8) or (classBytes[7].toInt() and 0xff)
    }

    private fun rewriteJarReplacingClass(
        source: File,
        dest: File,
        classPath: String,
        ...
        java.util.zip.ZipInputStream(source.inputStream().buffered()).use { zis ->
            java.util.zip.ZipOutputStream(tmp.outputStream().buffered()).use { zos ->
                var replaced = false
                while (true) {
                    val entry = zis.nextEntry ?: break
                    ...
                    zis.closeEntry()
                    val outEntry = java.util.zip.ZipEntry(name)
                    zos.putNextEntry(outEntry)
                    zos.write(data)
                    zos.closeEntry()
                }
                if (!replaced) {
                    val outEntry = java.util.zip.ZipEntry(classPath)
                    zos.putNextEntry(outEntry)
        ...
        if (dest.exists() && !dest.delete()) {
            error("cannot replace ${dest.absolutePath}")
        }
    ...
    private fun ensureCacio17(context: Context) {
        val destDir = cacio17Dir().also { it.mkdirs() }
        val marker = File(destDir, "version")
        val assetVer = runCatching {
            context.assets.open(ASSET_CACIO17_VERSION).bufferedReader().use { it.readText().trim() }
        }.getOrDefault("")
        val jars = listOf(
        ...
        for (name in jars) {
            runCatching {
                context.assets.open(ASSET_CACIO17_PREFIX + name).use { input ->
                    File(destDir, name).outputStream().use { output -> input.copyTo(output) }
                }
            }.onFailure {
                android.util.Log.e("BooxinRuntime", "cacio17 jar missing ($name): ${it.message}")
            }
        }
        if (assetVer.isNotEmpty()) marker.writeText(assetVer)
    }
    ...
    fun ensureNatives(context: Context) {
        ...
        if (markerOk && !missingRequired) {
            ...
            // 启动游戏前再准备 sqlite / 轻量别名，避免主界面冷启动卡死。
            return
        }
     ...
     * - [GlRendererKind.BOOXIN_GLUES]: 内置 Mesa Zink（libOSMesa_25.so），仅 26.3+
     ...
     * - [GlRendererKind.BOOXIN_ZINK] / Mesa (Zink / VirGL / Freedreno): keep holy gl4es
     *   as fallback; LIBGL_NAME points at OSMesa
     */
    ...
    fun applyRenderer(kind: GlRendererKind, profile: com.booxin.launcher.core.runtime.ModRenderProfile?) {
        ...
        when (kind) {
            ...
            GlRendererKind.BOOXIN_GLUES, GlRendererKind.BOOXIN_ZINK -> {
                val osmesa = File(dest, "libOSMesa_25.so")
                require(osmesa.isFile) { "缺少内置 BooxinGlues: ${osmesa.absolutePath}" }
                if (holy.isFile && !isGl4esSlotDisguise(holy, dest)) {
                    holy.copyTo(gl4, overwrite = true)
                }
                osmesa.setReadable(true, false)
                osmesa.setExecutable(true, false)
                android.util.Log.i(
                    "BooxinRuntime",
                    "BooxinGlues staged ${osmesa.name} (${osmesa.length()} bytes)"
                )
            }
            GlRendererKind.VULKAN_ZINK,
            GlRendererKind.VIRGL,
            GlRendererKind.FREEDRENO -> {
