...
| `BOOXIN_GLUES` | 内置 `libOSMesa_25.so`：Mesa OSMesa + Zink（MIT）；启动器编排 Apache-2.0 | Mesa3D + Booxin 接线 | **可闭源友好**（保留 MIT NOTICE）；**不**使用 MobileGlues 源码 | `assets/licenses/BOOXINGLUES_NOTICE.txt`、`MESA_MIT_LICENSE.txt` |
| `BOOXIN_ZINK` | 旧名，同 BooxinGlues | 同上 | 同上 | `assets/licenses/BOOXIN_ZINK_NOTICE.txt` |
